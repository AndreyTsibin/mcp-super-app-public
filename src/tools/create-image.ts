...
const OPENROUTER_DESCRIPTION =
  "Seedream 4.5 / Gemini 3 via OpenRouter. Returns the image inline in chat plus the saved paths and the measured cost. Seedream is the default model: $0.04 flat, up to 7.5MP, best prompt adherence — control framing with `aspect_ratio` and do NOT pass `size` (that only lowers the resolution for the same price). Seedream covers essentially every task; read the `model` description before picking anything else, it carries a measured decision table. EDITING: pass the source image via `reference_images` (local paths or URLs) plus an instruction in the prompt ('remove the sign', 'make the background lighter'); every model here accepts image input. Name what must stay unchanged explicitly ('keeping its pose unchanged') — the vendor-documented way to avoid drift. Mask-based inpainting is NOT supported. Sizing is model-specific: Seedream takes `aspect_ratio` alone; Gemini needs `aspect_ratio` + `resolution:'2K'`. Requires OPENROUTER_API_KEY in the server .env.";
