...
**Default model: `bytedance-seed/seedream-4.5`** — pass `aspect_ratio`, never `size`. Everything
in this file is text-carrying layout work, and 4.5 is the strongest model here at typography and
...
**Recommended model:** `bytedance-seed/seedream-4.5` — headline legibility and glass transparency both ride on prompt adherence, and 4.5 leads on both
**Args:** `aspect_ratio: '9:16'` · ~\$0.040

> Vertical costs pixels, not money: 9:16 returns well under the 7.5 MP seedream gives at 16:9,
> for the same \$0.040. Fine for a Story — it is a phone-screen deliverable.
...
**Recommended model:** `bytedance-seed/seedream-4.5` — text-heavy layout that must survive being shrunk to a feed thumbnail; dense-text rendering is 4.5's strongest suit
**Args:** `aspect_ratio: '1:1'` · ~\$0.040 (2048×2048, 4.2 MP)
...
**Recommended model:** `bytedance-seed/seedream-4.5` — bezel precision, small in-screen UI text and headline all need adherence; 4.5 is also the model documented to render UI from wireframes and annotations
**Args:** `aspect_ratio: '9:16'` · ~\$0.040
...
**Recommended model:** `bytedance-seed/seedream-4.5` — the densest text in this file (axis labels, KPI numbers, nav items) plus chart structure; 4.5 at 16:9 is also the best pixels-per-dollar on offer
**Args:** `aspect_ratio: '16:9'` · ~\$0.040 (3642×2048, 7.5 MP)
...
**Recommended model:** `bytedance-seed/seedream-4.5` — swatch colour accuracy, small labels throughout, and the same face repeated across three sections; 4.5 holds identity across panels better than the alternatives
**Args:** `aspect_ratio: '3:4'` · ~\$0.040
