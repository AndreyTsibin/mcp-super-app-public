...
Each pattern lists the exact `create_image` args to pass with it.
...
**Best fit for this pattern:** `bytedance-seed/seedream-5-0-lite` — the panel geometry and the "same person three times" requirement are prompt-following problems, and Seedream follows layout instructions best per dollar.
...
**Best fit for this pattern:** `google/gemini-3.1-flash-image` — this pattern lives or dies on skin. Flash renders pores and fine hairs; Seedream smooths them into a render-like finish.
...
**Best fit for this pattern:** `bytedance-seed/seedream-5-0-lite` — text rendering and the front/behind depth interplay are instruction-following, which is Seedream's strength. Quoted text is its documented syntax for on-image copy.
