...
Default model: `bytedance-seed/seedream-4.5` with `aspect_ratio` (never pass `size`). Any text that must appear in the image goes in double quotes — that is the syntax the model reads as "render this literally".
...
**Recommended model:** seedream 4.5 — the split-frame rule, the shared terrain and the single perspective across both halves are all prompt-adherence problems, and seedream follows them best per dollar.
...
**Recommended model:** seedream 4.5 — identity has to hold across three panels and the panel widths are an explicit instruction; both are prompt adherence.
**Args:** `model: 'bytedance-seed/seedream-4.5', aspect_ratio: '16:9'` → 3642×2048, \$0.040
...
**Recommended model:** seedream 4.5 — headline, subhead and on-screen UI all have to render as readable type, which is exactly what 4.5 improved. Use `4:5` for a POS or social hero, `16:9` for a site header.
...
**Recommended model:** seedream 4.5 — oversized type is the whole poster, and 4.5 handles typography and figure-over-letter layering best per dollar.
...
**Recommended model:** seedream 4.5 — mirror symmetry and species accuracy are prompt-adherence work, and at 7.5 MP it is also the largest frame on offer.
**Args:** `model: 'bytedance-seed/seedream-4.5', aspect_ratio: '3:4'` → \$0.040
