...
шага для человека (создать пустую папку и открыть её в Claude Code), потом готовый
текст, который копируется в чат целиком. Дальше агент делает всё сам — определяет
систему (macOS или Windows), проверяет окружение, скачивает и собирает сервер,
подключает его к Claude Code и чинит ошибки по дороге. Разбираться в коде не нужно.
