...
| `install_skill` | Ставит скилл в проект: bundled → копия в `.claude/skills/<id>/`; proxied → прогон официального CLI. Артефакты авто-добавляются в `.gitignore`. Каталог доступных скиллов (тип, назначение, команда установки для proxied) лежит прямо в описании параметра `skill`. |
