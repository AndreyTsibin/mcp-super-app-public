...
Пошаговая инструкция — в [INSTALL.md](INSTALL.md). Она из двух частей: сначала два
шага для человека (открыть новый чат в Claude Code — папку выбирать не нужно), потом
готовый текст, который копируется в чат целиком. Дальше агент делает всё сам — определяет
