...
Проверка обновлений кэшируется на сутки и полностью безопасна: нет сети или git —
сервер просто молчит и стартует как обычно. `update_server` отказывается работать,
если в папке сервера есть незакоммиченные правки — чужую работу он не трогает.
