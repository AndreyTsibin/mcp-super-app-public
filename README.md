...
| `create_image` | Единая точка входа для картинок: генерация и редактирование через OpenRouter. Возвращает **картинку в чат** + сохраняет файл в проект (`./generated` по умолчанию) + отдаёт `usage.cost`. Дефолт-модель — Seedream 5.0 Lite (`bytedance-seed/seedream-5-0-lite`, $0.035 за кадр при любом размере — от 7.5 МП по `aspect_ratio` до 16.8 МП с явным `size`), Gemini 3 Flash/Pro — через `model`. `reference_images` (пути/URL) — image-to-image, редактирование и консистентность серии. **Промпт обязателен через скилл `image`:** нет скилла в проекте или пустой `prompt_source` — тул отказывает и не тратит деньги. |
